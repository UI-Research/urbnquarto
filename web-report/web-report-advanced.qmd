...
csl: https://pandoc.org/demo/ieee.csl
format:
  html:
    theme: urbn.scss
...
*This document assumes basic familiarity with HTML, CSS, and JS. For an introduction, see these resources: Intro to HTML[@IntroHTML], Intro to DOM scripting using JavaScript[@IntroDOM], The HTML DOM Model[@HTMLDOM].*
...
::: {.callout-note}
### What is Bootstrap?
Bootstrap[@bootstrap] is a front-end framework designed for building responsive websites, using a combination of HTML, CSS, and JavaScript components. It exposes various CSS classes that can be used as-is or customized per a user's requirements. With JavaScript, you can add dynamic features to your Bootstrap CSS components and have more fine-grained control over how the interactive elements work in your Quarto report. 
:::

By combining Quarto's document authoring capabilities with custom CSS and JavaScript, you can deliver highly responsive and tailored web reports for your audience.[^2]

## Examples

### Two-column grid with an interactive dropdown button {#sec-interactive-dropdown}
...
#### Code: 
{% raw %}
```{.html filename="Two-column grid with an interactive dropdown button"} 
::::{.sample-grid}
:::{.col1}
This is a grid column with sample text.
:::
...
- An outer container is defined in Quarto Markdown using `.sample-grid` CSS class which contains two columnar sections, `.col1` and `.col2`.
...
- Font Awesome[@FontAwesome], an open source[@FontAwesomeLicense] icon library, is used to create the chevron (v-shaped arrow) icons (both upwards[@chevronUp] and downwards[@chevronDown]) for the dropdown button.
- The SVG symbol is defined in the `<svg>` block within the `include-in-header`[@quartoIncludeInHeader] section of the Quarto YAML front matter and can be identified globally using the `id` attribute (e.g., `icon-chevron-down`).

3. **Add a toggle button**
- A custom `.arrow-toggle` class is used to style the button in the second column (`.col2`).
- The button uses Bootstrap's `data-bs-toggle="collapse"` and `data-bs-target="#collapse-1"` attributes[@BootstrapCollapse] to toggle the visibility of the dropdown content using built-in JavaScript functionality.
...
- The button has an SVG chevron icon (`.chevron-icon`) that visually indicates toggle state to the end user by moving up/down whenever user clicks it.
...
#### Code: 
{% raw %}
~~~{.markdown filename="Two-Column Grid with an Interactive Dropdown Button"} 
<!-- Declare a Python function to generate customized cards with a collapsible element -->
...
<!-- Quarto Markdown to create a grid with a dropdown button -->
...
<!-- Call a Python function for generating card within the collapse -->
...
- Follow the steps detailed in @sec-interactive-dropdown above.

2. **Add accordion style collapsible element**
- For the collaspible element with `id="collapse-2"`, multiple accordion style `.card` elements are created using a Python function.[^1]
~~~{.markdown}
:::: {.collapse .attached-collapse id="collapse-2"}
...
::::
~~~
- `generate_cards()` function creates `n` Quarto `.card` elements which are styled using `custom-card` class defined in `styles.css`.
- Within each card element, an additional Bootstrap based collapse is added using the hyperlink `<a>` tag containing `data-bs-toggle="collapse"` and `data-bs-target` attributes[@BootstrapCollapse]. 
...
#### Code: 
{% raw %}
```{.html filename="Custom tooltip with overlay"} 
::::{.sample-grid-2}
This is an informative tooltip.
...
- The `custom-tooltip-trigger` class contains an SVG icon, `icon-info-circle`[@InfoCircle], which acts as the clickable element to display the tooltip content.
...
> **Note:** The `data-toggle-id` attribute is a user defined `data-*` attribute and is accessible as a `element.dataset.toggleId` element by JavaScript (hyphenated names map to camelCase).
...
[^1]: Analogous code can be written in R. A good place to start would be [this example](https://bookdown.org/yihui/rmarkdown-cookbook/results-asis.html) which leverages a combination of `asis` and cat. Note that this example is for RMarkdown and not Quarto, so, for Quarto, updates may be necessary. 
[^2]: Bootstrap components can also be used in `shiny` with the help of the `bslib` package as [this example](https://shiny.posit.co/blog/posts/bslib-dashboards/) shows.